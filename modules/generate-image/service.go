...
func (s *Service) GenerateImageWithGemini(ctx context.Context, base64Image string, prompt string, aspectRatio string) (string, error) {
	...
	// API 호출 (Vertex AI)
	log.Printf("📤 Sending request to Vertex AI with aspect-ratio: %s", aspectRatio)

	model := s.genaiClient.GenerativeModel(config.GeminiModel)
	model.SetTemperature(0.45)
	// Note: ResponseMIMEType should NOT be set for image generation with Gemini
	// The model will automatically return images in the appropriate format

	result, err := model.GenerateContent(ctx, parts...)
	if err != nil {
		return "", fmt.Errorf("Vertex AI API call failed: %w", err)
	}
...
func (s *Service) GenerateImageWithGeminiMultiple(ctx context.Context, categories *ImageCategories, userPrompt string, aspectRatio string) (string, error) {
	...
	if categories.Model != nil {
		...
		parts = append(parts, genai.Blob{MIMEType: "image/png", Data: resizedModel})
		log.Printf("📎 Added Model image (resized)")
	}

	if mergedClothing != nil {
		parts = append(parts, genai.Blob{MIMEType: "image/png", Data: mergedClothing})
		log.Printf("📎 Added Clothing image (merged from %d items)", len(categories.Clothing))
	}

	if mergedAccessories != nil {
		parts = append(parts, genai.Blob{MIMEType: "image/png", Data: mergedAccessories})
	...
	if categories.Background != nil {
		...
		parts = append(parts, genai.Blob{MIMEType: "image/png", Data: resizedBG})
	...
	// API 호출 (Vertex AI)
	log.Printf("📤 Sending request to Vertex AI with %d parts...", len(parts))
...
func (s *Service) DeductCredits(ctx context.Context, userID string, productionID string, attachIds []int) error {
	...
	if err != nil {
		log.Printf("⚠️ [GenerateImage] Failed to get user organization: %v", err)
	}
	...
	if isOrgCredit {
		log.Printf("💰 [GenerateImage] Deducting ORGANIZATION credits: OrgID=%s, User=%s, Images=%d, Total=%d credits", orgID, userID, len(attachIds), totalCredits)
	} else {
		log.Printf("💰 [GenerateImage] Deducting PERSONAL credits: User=%s, Images=%d, Total=%d credits", userID, len(attachIds), totalCredits)
	}
	...
	log.Printf("💰 [GenerateImage] Credit balance: %d → %d (-%d)", currentCredits, newBalance, totalCredits)
	...
	for _, attachID := range attachIds {
		...
		if err != nil {
			log.Printf("⚠️ [GenerateImage] Failed to record transaction for attach_id %d: %v", attachID, err)
		}
	}

	log.Printf("✅ [GenerateImage] Credits deducted successfully: %d credits from user %s", totalCredits, userID)
	return nil
}
